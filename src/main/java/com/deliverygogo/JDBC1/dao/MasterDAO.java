...
import com.deliverygogo.JDBC1.vo.OrderVO;
...
//                OrderVO vo = new OrderVO(CATEGORY_INFO, STORE_NUM, MENU_NAME, TOTAL);
    ...
    public void maslogin(MasterVO masterVO) {
        Connection conn = null;
        Statement stmt = null;
        PreparedStatement pStmt = null;
        ResultSet rs = null;
        String sql = "INSERT INTO MASTER_INFO(BIZNUM,MASTER_PW) VALUES(?,?)";

        try {
            conn = Common.getConnection();
            pStmt = conn.prepareStatement(sql);
            pStmt.setInt(1, masterVO.getBizNum());
            pStmt.setString(2, masterVO.getMasterPW());
            int ret = pStmt.executeUpdate();
            System.out.println("INSERT MASTER_INFO result : " + ret);
        } catch (Exception e) {
            e.printStackTrace();
        }
        Common.close(stmt);
